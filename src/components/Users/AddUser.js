...
function AddUser(){
  ...
  const [name,setName]=useState([])
      ...
      console.log(cookies.all.uroles[0])
  
  useEffect(()=>{

    if(cookies.all.uroles[0].users){
fetch(`${process.env.REACT_APP_API_BASE_URL}/user_roles.php?clie=${cookies.all.clie}}`)
...
fetch(`${process.env.REACT_APP_API_BASE_URL}/add_new_user.php?clie=${cookies.all.clie}`,{method:'post',header:{'Content-Type':'application/json'},body:JSON.stringify(body_data)})
.then((response)=>response.json())
.then((data)=>{
  if(data==200){
