...
const [cookies] = useCookies(['all']);

   const [value, setValue] = useState(2);
   const [input,setInput]=useState([]);

   const [label, setLabel] = useState(props.currstatus);
   ...
   useEffect(()=>{
    ...
    /*  let ondate2=ondate.getMonth()+1+'/'+ondate.getDate()+'/'+ondate.getFullYear();*/
      ...
      let ondate2=ondate.getFullYear()+'/'+(ondate.getMonth()+1)+'/'+ondate.getDate();

      console.log('date',ondate2);

    setInput((prev)=>({['tag']:tag,['lstatus']:label,['cmt']:'',['rating']:3,['lid']:props.data,['uid']:cookies.all.uid,['ondate']:ondate2}))
   },[])
   ...
   function handleChange(e){
    setLabel(e.target.value)
     ...
     if (e.target.value == 'DISQUALIFIED' || e.target.value == 'OPPORTUNITY LOST') {
   ...
   function handleDateTimeChange(e,t){
    ...
    if(t=='date'){
     /* let y=dat.getMonth()+1+'/'+dat.getDate()+'/'+dat.getFullYear();*/
      ...
      let y=dat.getFullYear()+'/'+(dat.getMonth()+1)+'/'+dat.getDate();
  ...
  setInput((prev)=>({...prev,[name]:value}))
   ...
   function submit_form(e){
      e.preventDefault();
       ...
       if (isDateRequired && !input.date) {
    setAlert({ severity: 'error', msg: 'Date is required' });
    setSnack(1);
    return;
  }
   ...
   let cleanInput = {
    ...
    cmt: (input.cmt || "")
      ...
      let postdata=JSON.stringify(cleanInput);

   fetch(`${process.env.REACT_APP_API_BASE_URL}/update_followup.php`,{
    method:'post',
...
<DatePicker label="Followup Date" sx={{margin:2,width:'12.5vw'}} name="nxtfoll" value="" onChange={(e,c)=>handleDateTimeChange(e,'date')}  slotProps={{
        textField: {
